...
const FilteredExerciseCard = () => {
  const specificBodyPartData = useSelector(
    (state) => state.new.specifichBodyPartData
  );

  return (
    <>
      {specificBodyPartData &&
        specificBodyPartData?.map((exercise) => (
          <Link key={exercise.id} to={`/exercises/exercise/${exercise.id}`}>
            <div className="w-72 md:w-80 relative my-5 md:my-10 rounded-2xl bg-white bg-opacity-25">
              ...
              <div className="font-semibold text-lg md:text-xl mt-4 mb-1 px-5 py-2">
...
export default FilteredExerciseCard;
